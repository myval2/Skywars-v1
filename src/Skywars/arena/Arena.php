...
class Arena implements Listener {
    ...
    public function joinToArena(Player $player) {
        if(!$this->data["enabled"]) {
            $player->sendMessage("§c> Arena is currently installed");
            return;
        }
        ...
        if($this->inGame($player)) {
            $player->sendMessage("§c> You're Alright!");
            return;
        }
        ...
        $this->broadcastMessage("§4> {$player->getName()} Join the Game! §4[".count($this->players)."/{$this->data["slots"]}]");
    }
    ...
    public function disconnectPlayer(Player $player, string $quitMsg = "", bool $death = \false) {
        ...
        if(!$death) {
            $this->broadcastMessage("§4> {$player->getName()} Left The Game. §4[".count($this->players)."/{$this->data["slots"]}]");
        }

        if($quitMsg != "") {
            $player->sendMessage("§4> $quitMsg");
        }
    }
    ...
    public function startGame() {
        ...
        $this->broadcastMessage("Game has started!", self::MSG_TITLE);
    }
    ...
    public function startRestart() {
        ...
        $player->addTitle("§ayou win!");
        $this->plugin->getServer()->getPluginManager()->callEvent(new PlayerArenaWinEvent($this->plugin, $player, $this));
        $this->plugin->getServer()->broadcastMessage("§4[SkyWars] {$player->getName()} Player Name {$this->level->getFolderName()} Won The Map!");
        $this->phase = self::PHASE_RESTART;
    }
    ...
    public function onInteract(PlayerInteractEvent $event) {
        ...
        if($this->phase == self::PHASE_GAME) {
            $player->sendMessage("§c> Arena Continues");
            return;
        }
        if($this->phase == self::PHASE_RESTART) {
            $player->sendMessage("§c> Restarting!");
            return;
        }
    ...
    public function loadArena(bool $restart = false) {
        if(!$this->data["enabled"]) {
            $this->plugin->getLogger()->error("Arena Could not load: Arena Not active");
            return;
        }
